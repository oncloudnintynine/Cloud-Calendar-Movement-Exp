...
setTimeout(() => el.classList.add('hidden-view'), 5000);
}
...
pill.id = 'sync-pill';
pill.className = 'fixed bottom-[70px] left-1/2 transform -translate-x-1/2 z-[90] rounded-full shadow-lg text-xs font-bold px-4 py-2 transition-all duration-300 pointer-events-none opacity-0 translate-y-10 flex items-center space-x-2';
document.body.appendChild(pill);
}

if (window.isOffline && window.syncQueue.length > 0) {
pill.className = 'fixed bottom-[70px] left-1/2 transform -translate-x-1/2 z-[90] rounded-full shadow-lg text-xs font-bold px-4 py-2 transition-all duration-300 pointer-events-none flex items-center space-x-2 bg-red-600 text-white opacity-100 translate-y-0';
...
pill.className = 'fixed bottom-[70px] left-1/2 transform -translate-x-1/2 z-[90] rounded-full shadow-lg text-xs font-bold px-4 py-2 transition-all duration-300 pointer-events-none flex items-center space-x-2 bg-blue-600 text-white opacity-100 translate-y-0';
...
// Just finished
pill.className = 'fixed bottom-[70px] left-1/2 transform -translate-x-1/2 z-[90] rounded-full shadow-lg text-xs font-bold px-4 py-2 transition-all duration-300 pointer-events-none flex items-center space-x-2 bg-emerald-600 text-white opacity-100 translate-y-0';
...
try {
   ...
   if (e.message.includes('Unauthorized') || e.message.includes('fatal')) {
       window.syncQueue.shift(); 
       if (window.showToast) window.showToast("A background save was rejected: " + e.message, "error");
       else alert("A background save was rejected by the server: " + e.message);
   } else {
